          ...
          <td align="center">
            <a href="https://www.paypal.com/in/webapps/mpp/paypal-popup" title="How PayPal Works"
              onclick="javascript:window.open('https://www.paypal.com/in/webapps/mpp/paypal-popup','WIPaypal','toolbar=no, location=no, directories=no, status=no, menubar=no, scrollbars=yes, resizable=yes, width=1060, height=700'); return false;"><img
                src="{{ asset('frontend/') }}/payment/others.png"  width="200px;" border="0"
                alt="EdukanPay"></a>
          </td>
        </tr>
      </table>
      <form action="{{ route('edokan.payment') }}"  method="post" >
        @csrf
        ...
        @if($type == 'payment')
          ...
          <input type="hidden" name="amount" readonly value="{{ $type == 'payment' ? number_format($data['price'] , 2) :''}}">
          ...
          <button type="submit" class="btn btn-success">Pay ${{ number_format($data['price'] , 2) }} from Others</button>
        ...
        @elseif($type == 'recharge')
          ...
          <button type="submit" class="btn btn-success">Pay ${{ number_format($data['amount'] , 2) }} from Others</button>
        @elseif($type == 'subscribe')

          @if($data['is_lifetime'])
          <input type="hidden" name="amount" value="{{ $data['total_subscription_fee'] }}" readonly>
        ...
        <input type="hidden" name="total_month" value="{{ $data['total_month'] }}">
          ...
          <button type="submit" class="btn btn-success">Pay ${{ number_format($data['total_subscription_fee'] , 2) }} from Others</button>
        @elseif ($type =='customerrequestEdoken')
        <input type="hidden" name="orderid" value="{{ $booking->id }}">
        <input type="hidden" name="amount" value="{{ $amount }}">
        <input type="hidden" name="type" value="{{ $type }}">
        <input type="hidden" name="name" value="{{ $name }}">
        <input type="hidden" name="email" value="{{ $email }}">
        <button type="submit" class="btn btn-success">Pay ${{ $amount }} from Others</button>
