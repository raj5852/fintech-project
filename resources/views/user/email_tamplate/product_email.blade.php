                                                ...
                                                <table width="100%" border="0" cellspacing="0" cellpadding="0">
                                                    <tr>
                                                        <th class="column" style="font-size:0pt; line-height:0pt; padding:0; margin:0; font-weight:normal;">
                                                            <table width="100%" border="0" cellspacing="0" cellpadding="0">
                                                                <tr>
                                                                    <td class="img m-center" style="font-size:0pt; line-height:0pt; text-align:left;"><img src="{{asset('backend/setting/'.$setting->image) }}" width="100" height="44" border="0" alt="" /></td>
                                                                </tr>
                                                            </table>
                                                        </th>
                                    ...
                                    <table width="100%" border="0" cellspacing="0" cellpadding="0" bgcolor="#ffffff">
                                        <tr>
                                            <td class="p30-15" style="padding: 30px 30px 70px 30px;">
                                                <table width="100%" border="0" cellspacing="0" cellpadding="0">
                                                    <tr>
                                                        <td style="color:#000000; font-family:'Ubuntu', Arial,sans-serif; font-size:14px; line-height:20px; padding-bottom:10px;">
                                                            <p><span style="font-weight: bold; text-decoration: underline;">Here is your product links</span></p>
                                                            @foreach($emailContent['product_url'] as $key=> $link)
                                                            @if ($emailContent['product_name'])
                                                             <p><b>Name:  {{$emailContent['product_name'][$key]}}</b></p>

                                                            @endif

                                                            @if(filter_var($link,FILTER_VALIDATE_URL))
                                                                <p><a target="_blank" href="{{ $link }}"> {{ substr( $link , 0, 100) }}....</a></p>

                                                            @else
                                                            <p><a target="_blank" href="{{ decrypt($link) }}"> {{ substr( decrypt($link) , 0, 100) }}....</a></p>
                                                            @endif
